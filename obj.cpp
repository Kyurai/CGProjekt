...
bool ReadMaterialLibrary(const std::string& filename, vector<Material*>& materials) {
    std::ifstream fp(filename.c_str(),std::ios::in);
    if(!fp)
        return false;
    std::string tmp(std::istreambuf_iterator<char>(fp), (std::istreambuf_iterator<char>()));
    ...
    while(std::getline(buffer, line)) {
        ...
        if(line.find_first_of("#") != std::string::npos) //its a comment leave it
            continue;
        if(line.length()==0)
            continue;
...
bool ObjLoader::Load(const std::string& filename, std::vector<Mesh*>& meshes, std::vector<Vertex>& verts, std::vector<unsigned short>& indices,	std::vector<Material*>& materials, BBox& bbox, std::vector<glm::vec3>& verts2, std::vector<unsigned short>& indices2) {
    std::ifstream fp(filename.c_str(),std::ios::in);
    if(!fp)
        return false;
    std::string tmp(std::istreambuf_iterator<char>(fp), (std::istreambuf_iterator<char>()));
    ...
    while(std::getline(buffer, line)) {
        ...
        if(prefix.compare("mtllib")==0) {
            ...
            std::string full_path = filename.substr(0, filename.find_last_of("/")+1);
